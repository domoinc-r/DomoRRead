...
.onLoad <- function(libname, pkgname) {
  packageStartupMessage("Welcome to DomoRRead")
}
...
#' @export
#' @examples
#' DomoRRead::init(Sys.getenv('DOMO_BASE_URL'), Sys.getenv('DEVELOPER_TOKEN'))
init <- function(customer,
  ...
  if (!requireNamespace("httr", quietly = TRUE)) {
    stop("'httr' is needed for the DomoRRead package", call. = FALSE)
  }
  
  # check we have pryr
  if (!requireNamespace("pryr", quietly = TRUE)) {
    stop("'pryr' is needed for the DomoRRead package", call. = FALSE)
  }
  ...
  .domo_env$user.agent <- c("User-Agent"="DomoRRead-test/1.0")
